package lab2.Davydzenko.gr10.varA9;
...
public class MainFrame extends JFrame {
    ...
    public Double calculate1(Double x, Double y, Double z) {
        return (Math.sin(Math.sin(y) + Math.pow(2.718, Math.cos(y)) + z*z) * Math.pow(Math.sin(Math.PI*y*y) + Math.log(x*x),0.25));
    }
    ...
    public MainFrame() {
        ...
        hboxVariables.add(Box.createHorizontalStrut(100)); // Добавить «распорку» C2-H5 шириной 100 пикселов для отступа между текстовым полем для ввода Y и подписью для Z
        hboxVariables.add(LabelForZ); // Добавить подпись для переменной Z
        hboxVariables.add(Box.createHorizontalStrut(10)); // Добавить «распорку» C2-H6 шириной 10 пикселов для отступа между надписью и текстовым полем для ввода значения Z
        ...
        hboxResult.add(Box.createHorizontalGlue());// Создать контейнер «коробка с горизонтальной укладкой»
        ...
        hboxResult.add(Box.createHorizontalStrut(150)); // Добавить «распорку» C3-H2 в 10 пикселов между подписью и полем результата
        hboxResult.add(Box.createVerticalGlue());
